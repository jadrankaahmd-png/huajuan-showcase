'use client';

import { SatelliteData } from '../types';

interface SatelliteModuleProps {
  satelliteData: SatelliteData[];
}

export default function SatelliteModule({ satelliteData }: SatelliteModuleProps) {
  return (
    <div className="mb-8">
      <h2 className="text-xl font-bold text-gray-900 mb-4 flex items-center gap-2">
        <span>🛰️</span>
        <span>卫星火点（NASA FIRMS API）</span>
        <span className="text-xs bg-yellow-50 text-yellow-600 px-2 py-1 rounded">⚠️ 暂时无法获取</span>
      </h2>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {satelliteData.map((item, index) => (
          <div key={index} className="bg-yellow-50 rounded-lg shadow-sm p-4 border border-yellow-200">
            <div className="flex items-center justify-between mb-2">
              <div className="text-sm font-semibold text-gray-900">{item.location}</div>
              <span className="text-sm">{item.available ? '🔥' : '⚠️'}</span>
            </div>
            <div className="text-sm text-gray-600 mb-1">
              <span className="font-medium">火点数量: </span>
              {item.available ? item.firePoints : '暂时无法获取'}
            </div>
            <div className="text-sm text-gray-600 mb-2">
              <span className="font-medium">强度: </span>
              {item.intensity}
            </div>
            <div className="text-xs text-gray-400">最后更新：{item.lastUpdate}</div>
          </div>
        ))}
      </div>
    </div>
  );
}
