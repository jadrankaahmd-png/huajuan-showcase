...
export async function GET() {
  try {
    // 从 Redis 读取第三层能力
    const capabilities = await redis.get('layer3:capabilities') || [];
    const stats = await redis.get('layer3:stats') || {
      total: 0,
      active: 0,
      inactive: 0,
      progress: 0
    ...
    console.error('读取第三层能力失败:', error);
    return NextResponse.json({
      error: '读取失败',
      capabilities: [],
      stats: { total: 0, active: 0, inactive: 0, progress: 0 }
    }, { status: 500 });
  }
}
